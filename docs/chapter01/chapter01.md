...
### 1.3.4 过程作为返回值
**牛顿法**  
&emsp;&emsp;如果$x \mapsto g(x)$是一个可微函数，那么方程$g(x)=0$的一个解就是函数$x \mapsto f(x)$的一个不动点，其中：$$f(x) = x - \frac{g(x)}{Dg(x)}$$&emsp;&emsp;这里的$Dg(x)$是$g$对$x$的导数。牛顿法就是使用不动点方法，通过搜寻函数$f$的不动点的方式，去逼近上述方程的解。  
&emsp;&emsp;如果$g$是一个函数，且$dx$是一个很小的数，那么$g$的导数在任一数值$x$的值由下面函数（作为很小的数$dx$的极限）给出：$$Dg(x)=\frac{g(x+dx)-g(x)}{dx}$$&emsp;&emsp;代码见`src\examples\ch01\p49-deriv.scm`  
&emsp;&emsp;有了`deriv`函数，牛顿法可以表述为一个求不动点的过程，代码见`src\examples\ch01\p49-newtons-method.scm`  

**抽象和第一级过程**  
&emsp;&emsp;一般而言，程序设计语言总会对计算元素的可能使用方法强加上某些限制。带有最少限制的元素被称为具有第一级的状态。第一级元素的某些“权利或者特权”包括：
- 可以用变量命名
- 可以提供给过程作为参数
- 可以由过程作为结果返回
- 可以包含在数据结构中